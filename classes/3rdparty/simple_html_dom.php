...
class simple_html_dom_node {
	...
	function find($selector, $idx=null, $length=0)
	{
		...
		if(isset($found[$idx]))
		{
			// If the count is negative, pull off that many from the end.
			if($length < 0)
			{
				$count	= ($iFound + $length) - $idx;
			}
			...
			// Else we want the index to the end
			else if($length == 0)
			{
				return array_slice($found, $idx);
			}
			// Else we want a specific slice
			else
			{
				if($idx + $length < $iFound)
				{
					return array_slice($found, $idx, $length);
				}
			}
		}
...
class simple_html_dom {
    ...
    function find($selector, $idx=null, $length=0) {
        return $this->root->find($selector, $idx, $length);
    }
